...
# ND和PSP组间差异的
a <- read.csv("Genus_HFD_PSP_P0.05.csv", header = T, row.names = 1,check.names = F)
# ND和HFD组间差异的
b <- read.csv("Genus_ND_HFD_P0.05.csv", header = T, row.names = 1,check.names = F)
# HFD和PSP组间差异的
c <- read.csv("Genus_ND_PSP_P0.05.csv", header = T, row.names = 1,check.names = F)
...
names(abc) <- c("HFD_PSP", "ND_HFD", "ND_PSP")
...
intersect_genus <- Reduce(intersect,  list(v1 = a1,
                                           v2 = a2,
                                           v3 = a3))
intersect_genus2 <- intersect(a1, a2) # 求a2和a3的交集
...
venn.diagram(
  ...
  filename = "Plots/venn_ND_HFD_PSP2_genus.png",
  output = TRUE ,
  imagetype="png" ,
  height = 1200 , 
  width = 1200 , 
...
library(cowplot)
b <- read.csv("taxa_abundance/Absolute/otu_table.g.absolute.csv",
...
ggsave(pp, file = "Plots/Genus_intersect_ND_HFD_PSP_boxplot4.png", height = 3, width = 10, dpi = 300)
