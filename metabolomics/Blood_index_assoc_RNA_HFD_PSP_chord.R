...
a <- read.csv(paste(temp_dir,"/Blood_biochemical_indicators.csv",sep = ""), 
              header = T,row.names = 1, check.names = F)
a2 <- a[7:nrow(a), ]
# 对生化指标进行Z-score转化，消除量纲的影响
a_temp <- scale(a2)
a_temp <- as.data.frame(a_temp)
dat <- a_temp


# 差异的基因
temp_dir <- "F:\\其他人员\\朱亚玲\\PSP-姜黄素-生化指标测定\\"
b <- read.csv(paste(temp_dir,"/RNA_contrary_in_ND_HFD_PSP_all_38genes.csv",sep = ""), 
              header = T,row.names = 1, check.names = F)
b2 <- b[, 7:17]
...
library(psych)
library(reshape2)
corr_df <- corr.test(dat, b3, method = "spearman", adjust = "BH", alpha = .05)
...
write.csv(nbar.m,"assoc_results/blood_index_HFD_PSP_assoc_RNA.csv") 
# 结果太少，暂时不作图
...
png(file = "assoc_results/metabo_RNA_HFD_PSP_circlize202305011.png", height = 2400, width = 2400,
    res = 300)
...
circos.par(start.degree = 0, gap.degree = 1, points.overflow.warning = FALSE,
           ...
           circle.margin = c(0.1,0.1,0.1,0.1)) #controls the margins on the left, right, bottom and top sides of the circle
#par(mar = rep(-2, 4))
#par(mar = c(-8,-8,0,0))
...
title(paste("Metabolites assocaited with" ,"genes", sep = " "))
